...
With a numeric prefix \"0\", it displays the context of the
current clock or the last clock item.
...
rebuilds the clock history and lets you browse it."
  ...
  (pcase arg
    ('nil (cond
           ...
           (t (org-clock-goto))))
    (0 (counsel-org-clock-context (if (org-clocking-p)
                                      org-clock-marker
                                    (or (car org-clock-history)
                                        (user-error "Empty clock history")))))
